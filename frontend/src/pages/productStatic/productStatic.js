...
const ProductStatic = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [productsRes, detailsRes] = await Promise.all([
        ...
        axios.get('http://localhost:3001/bill_detail'),
      ]);
      ...
      const salesMap = {};
      details.forEach(d => {
        salesMap[d.product_id] = (salesMap[d.product_id] || 0) + d.product_quantity;
      });
      ...
      const sorted = stats.sort((a, b) =>
        sortOrder === 'desc' ? b.total_sold - a.total_sold : a.total_sold - b.total_sold
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const billDetailRes = await axios.get("http://localhost:3001/bill_detail");
      ...
      const mergedStats = productsRes.data.map((product) => {
        ...
        const totalSold = relatedDetails.reduce((sum, d) => sum + d.product_quantity, 0);
        const totalRevenue = relatedDetails.reduce((sum, d) => sum + d.sub_total, 0);
  ...
  useEffect(() => {
    const totalRev = statistics.reduce((sum, item) => sum + item.totalRevenue, 0);
  ...
  const handleExportCSV = () => {
    const headers = ["ID", "Tên sản phẩm", "Số lượng đã bán", "Doanh thu (VND)"];
            ...
            <div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", margin: "20px 0" }}>
                  ...
                  <input
                    ...
                    style={{ padding: "8px", borderRadius: "6px", border: "1px solid #ccc", width: "160px" }}
                  />
                  <input
                    type="number"
                    ...
                    style={{ padding: "8px", borderRadius: "6px", border: "1px solid #ccc", width: "160px" }}
                  />
                  <button onClick={handleExportCSV} style={{ padding: "8px 16px", borderRadius: "6px", backgroundColor: "#4CAF50", color: "white", border: "none" }}>
                    📥 Xuất CSV
                  </button>
                </div>

                <table border="1" cellPadding="8" style={{ borderCollapse: "collapse", marginTop: "10px", width: "100%", backgroundColor: "white" }}>
                  <thead>
                    <tr>
                      <th>ID</th>
                      <th>Tên sản phẩm</th>
                      <th style={{ cursor: "pointer" }} onClick={() => handleSort("sold")}>
                        Số lượng đã bán {sortBy === "sold" ? (sortOrder === "asc" ? "⬆️" : "⬇️") : ""}
                      </th>
                      <th style={{ cursor: "pointer" }} onClick={() => handleSort("revenue")}>
                        Doanh thu (VND) {sortBy === "revenue" ? (sortOrder === "asc" ? "⬆️" : "⬇️") : ""}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
          ...
          return (
            <div style={{
              ...
              <table style={{
                ...
                <tbody>
                  {productStats.map(p => (
                    <tr key={p.product_id} style={{ backgroundColor: '#fff', borderBottom: '1px solid #ccc' }}>
                      <td style={{ padding: '10px', textAlign: 'center' }}>{p.product_id}</td>
                      <td style={{ padding: '10px' }}>{p.product_name}</td>
                      <td style={{ padding: '10px', textAlign: 'center' }}>{p.total_sold}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody>
                  {sortedProducts.map(p => (
                    <tr key={p.product_id} style={{ backgroundColor: '#fff', borderBottom: '1px solid #ccc' }}>
                      <td style={{ padding: '10px', textAlign: 'center' }}>{p.product_id}</td>
                      <td style={{ padding: '10px' }}>{p.product_name}</td>
                      <td style={{ padding: '10px', textAlign: 'center' }}>{p.product_inventory_quantity}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <button onClick={() => setActiveTab('overview')} style={{ marginRight: '10px' }}>Tổng quan sản phẩm</button>
          <button onClick={() => setActiveTab('popular')} style={{ marginRight: '10px' }}>Sản phẩm phổ biến</button>
          <button onClick={() => setActiveTab('inventory')}>Báo cáo tồn kho</button>
          </div>
          {renderContent()}
        </div>
    </div>
